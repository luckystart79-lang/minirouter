...
export default function App() {
  ...
  // Save config whenever quota changes
  useEffect(() => {
    if (quota !== null && lastResetDate !== null) {
      ipcRenderer.invoke('save-config', { quota, lastResetDate });
    }
  }, [quota, lastResetDate]);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      const apps = await ipcRenderer.invoke('check-active-apps');
    ...
    <div style={{ display: 'flex', height: '100vh' }}>
      ...
      <div style={{ flex: 1, padding: '20px', background: '#ecf0f1' }}>
        {activeTab === 'dashboard' ? (
          ...
          <Learning onEarnBonus={addBonusTime} />
        )}
      </div>
    </div>
  );
